...
safe_eval_with_capture <- function(code_string, output_dir, task_num) {
  ...
  code_file <- file.path(
    output_dir,
    stringr::str_glue("task_{task_num}_code.R")
  )
  writeLines(code_string, code_file)

  output_file <- file.path(
    output_dir,
    stringr::str_glue("task_{task_num}_output.txt")
  )
  plot_file <- file.path(
    output_dir,
    stringr::str_glue("task_{task_num}_plot.png")
  )
...
generate_analysis_code <- function(
  task,
  data_name,
  previous_error = NULL,
  model = "claude",
  ...
  package_info <- if (
    !is.null(installed_packages) && length(installed_packages) > 0
  ) {
    stringr::str_c(
      "This system has ",
  ...
  prompt <- stringr::str_c(
    "Generate R code to perform this analysis task on the dataset '",
    data_name,
    "':\n",
    task,
    "\n\n",
...
create_summary_report <- function(
  ...
  for (i in seq_along(tasks)) {
    ...
    report_lines <- c(
      ...
      if (!is.null(result$code_file)) {
        stringr::str_glue("- Code: `{basename(result$code_file)}`")
      } else {
        NULL
      },
      if (!is.null(result$output_file)) {
        stringr::str_glue("- Output: `{basename(result$output_file)}`")
      } else {
        NULL
      },
      if (!is.null(result$plot_file)) {
        stringr::str_glue("- Plot: `{basename(result$plot_file)}`")
      } else {
        NULL
      },
      if (!is.null(result$error)) {
        stringr::str_glue("- Error: {result$error}")
      } else {
        NULL
      },
      ""
...
auto_eda_ellmer <- function(
  ...
  if (is.null(output_dir)) {
    ...
    output_dir <- file.path(
      "auto_eda_ellmer_output",
      stringr::str_glue("{data_name}_{timestamp}")
    )
  }
  ...
  if (length(tasks) == 0) {
    cat(
      "⚠️  No tasks found in the analysis plan. Please check the LLM response.\n"
    )
    ...
    purrr::imap(function(task_raw, i) {
      ...
      for (attempt in 1:max_attempts) {
        ...
        if (
          !stringr::str_detect(
            review,
            stringr::regex("APPROVED", ignore_case = TRUE)
          )
        ) {
...
cat(
  "Ellmer supports: Claude, OpenAI, Gemini, Ollama, and many more providers\n"
)
